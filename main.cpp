...
int main()
{
	...
	//8 indicates the size of one plaintext
	int numberOfPlainText = plainText.length() / sizeOfplainText;
	int bitsRequired = plainText.length() % sizeOfplainText;
	
	if (plainText.length() < sizeOfplainText)
	{
		bitsRequired = sizeOfplainText - plainText.length();
	}
	if (bitsRequired != 0)
	{
		numberOfPlainText++;
	...
	temp += plainText;
	plainText = temp;
	for (int i = 0; i < numberOfPlainText; i++)
	{
		temp = "";
